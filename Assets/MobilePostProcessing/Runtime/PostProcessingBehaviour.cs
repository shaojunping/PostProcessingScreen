...
namespace UnityEngine.PostProcessing
{
    ...
    public class PostProcessingBehaviour : MonoBehaviour
    {
        ...
        WaterEffectComponent m_WaterEffect;
        OutLineComponent m_OutLine;
        SkyScatterComponent m_SkyScatter;
        //DitheringComponent m_Dithering;
        //FxaaComponent m_Fxaa;

        CommandBuffer renderCommand;
        public Renderer[] outRen;
        //public bool refleshOutRen = true;

        public Transform sunShaftTran;
        //
        ...
        void OnEnable()
        {
            ...
            m_WaterEffect = AddComponent(new WaterEffectComponent());
            m_OutLine = AddComponent(new OutLineComponent());
            m_SkyScatter = AddComponent(new SkyScatterComponent());
            //m_Dithering = AddComponent(new DitheringComponent());
            //m_Fxaa = AddComponent(new FxaaComponent());

            //--------------------------------------------------------------
            renderCommand = new CommandBuffer();
            renderCommand.name = "Render Solid Color Outliner";
            //--------------------------------------------------------------
        ...
        void OnPreCull()
        {
            ...
            m_WaterEffect.Init(context, profile.waterEffect);
            m_OutLine.Init(context, profile.outLine);
            m_SkyScatter.Init(context, profile.skyScatter);
        ...
        void OnPreRender()
        {
            ...
            if (m_OutLine.active)
            {
                ...
                var m_outterLineMat = m_MaterialFactory.Get("Unlit/ColorSel");
                //Debug.Log("renderer:" + outRen.name);
                foreach (Renderer ren in outRen)
                    renderCommand.DrawRenderer(ren, m_outterLineMat);
                //refleshOutRen = false;
            }

            //TryExecuteCommandBuffer(m_OutLine);
        ...
        void OnRenderImage(RenderTexture source, RenderTexture destination)
        {
            ...
            {
                uberActive = true;
                //Sky Scatter in Bloom pass
                m_Bloom.Prepare(src, uberMaterial, autoExposure);
            }


            if (m_SkyScatter.active)
            {
                uberActive = true;
                m_SkyScatter.Prepare(src, uberMaterial, m_Camera, sunShaftTran);
            }

            if (m_OutLine.active)
            {
                uberActive = true;
                m_OutLine.Prepare(src, uberMaterial, renderCommand);
            }
            ...
            uberActive |= TryPrepareUberImageEffect(m_WaterEffect, uberMaterial);
        ...
        void OnGUI()
        {
           ...
           if (m_ColorGrading.active && profile.debugViews.IsModeActive(DebugMode.LogLut))
